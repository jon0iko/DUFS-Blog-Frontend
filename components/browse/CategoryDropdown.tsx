...
export default function CategoryDropdown({
  ...
  return (
    <div ref={dropdownRef} className="relative w-full lg:w-auto lg:min-w-[240px]">
      <button
        ref={buttonRef}
        onClick={() => setIsOpen(!isOpen)}
        className={cn(
          "h-12 w-full appearance-none rounded-[4px] border border-[#BDB2AB] bg-[#C9C0BC] px-4 pr-10 text-[1.1rem] lg:text-[1.2rem] font-black text-[#29211D] outline-none transition-all focus:border-[#84786F] dark:border-[#3A3431] dark:bg-[#302A27] dark:text-[#F3E7DD] cursor-pointer flex items-center justify-between"
        )}
      >
        <span className="flex items-center gap-1 flex-1 text-left">
          {bn && <span className={cn(getFontClass(bn))}>{bn}</span>}
          {en && <span className="font-montserrat">{en}</span>}
        </span>
        <ChevronDown
          className={cn(
            ...
            {options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full px-4 py-3 text-left text-[1rem] lg:text-[1.1rem] font-black transition-colors flex items-center gap-1 hover:opacity-90",
                    isSelected
                ...
                >
                  ...
                  {optEn && <span className="font-montserrat">{optEn}</span>}
                </button>
              );
            })}
          </div>
