...
import type { Publication_Issue } from '@/types';
...
function PiecesInner() {
  ...
  const issueId = useMemo(() => searchParams.get('issue') || '', [searchParams]);
  ...
  useEffect(() => {
    ...
    const fetchIssue = async () => {
      setLoading(true);
      try {
        const issueData = await strapiAPI.getIssueWithPieces(issueId);
        if (issueData) {
          setIssue(issueData);
        }
      } catch (err) {
  ...
  return (
    <div className="min-h-screen pb-24 relative overflow-hidden bg-background">
      ...
      <div className="container relative z-10 pt-16">
        ...
        <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center md:items-start mb-16 bg-card p-8 rounded-2xl border border-border shadow-md">
          ...
          <div className="flex-1 flex flex-col justify-center h-full">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-secondary text-secondary-foreground text-xs font-bold uppercase tracking-widest mb-6 w-max border border-border">
              <BookOpen className="w-3.5 h-3.5" /> Published {new Date(issue.PublishedDate).getFullYear()}
            </div>
            ...
            <div className="bg-primary/10 border border-primary/20 rounded-xl p-6 mb-8">
              <p className="text-primary-foreground font-semibold text-base md:text-lg flex items-start gap-3">
        ...
        {/* Selected Writings */}
        {issue.Pieces && issue.Pieces.length > 0 && (
          <div className="mt-16">
            <div className="flex flex-col items-center text-center mb-12">
              <h2 className="text-3xl md:text-4xl font-black tracking-tight mb-4">Selected Writings</h2>
              <p className="text-muted-foreground max-w-2xl">
                A glimpse into the contents of this issue. Read these pieces online, and collect the physical copy for more.
              </p>
            </div>
